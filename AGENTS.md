...
- `app/main.py`
  ...
  - UI 路由：`/`、`/ui/upload`、`/ui/upload/batch`、`/ui/chat`、`/ui/grounding`、`/ui/documents/manage`、`/ui/documents`。
...
- `API_PUBLIC_PORT`, `QDRANT_PUBLIC_PORT`, `API_IMAGE_TAG`, `EMBEDDING_PUBLIC_PORT`
- `GUNICORN_WORKERS`, `GUNICORN_THREADS`
- `EMBEDDING_GGUF_HOST_PATH`, `EMBEDDING_GGUF_FILE`, `EMBEDDING_CONTEXT_WINDOW`, `EMBEDDING_SERVER_BATCH`, `EMBEDDING_SERVER_PARALLEL`, `EMBEDDING_UBATCH`, `EMBEDDING_THREADS_BATCH`, `EMBEDDING_MAX_INPUT_TOKENS`
- `SELF_HOSTED_EMBEDDING_MODEL`, `SELF_HOSTED_EMBEDDING_DIM`, `EMBEDDING_COLLECTION_NAME`
补充：当前 Vulkan 自托管 Embedding 默认推荐 `EMBEDDING_SERVER_BATCH=1024`、`EMBEDDING_SERVER_PARALLEL=2`、`EMBEDDING_UBATCH=1024`、`EMBEDDING_THREADS_BATCH=8`，并配合 `EMBEDDING_MAX_INPUT_TOKENS≈3500` 做入库前准入。
